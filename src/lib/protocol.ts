...
- API ERRORS ARE HANDLED BY THE MESH, NOT BY YOU. Agents sometimes get killed by an Anthropic API error (overloaded / rate-limited), which can hit one agent, several, or everyone at once. The SERVER detects this and revives the agent automatically on a 2→4→8min backoff — it is 100% code and needs no agent to act. So when the mesh tells you "X is DOWN — API error" or "your leader is DOWN": do NOT try to revive it, do NOT spam it with messages, do NOT sit idle waiting on it. The mesh will bring it back and tell you when it has ("X recovered"). The ONLY judgment call that's yours: if X's task genuinely can't wait, reassign it to someone present; otherwise just keep doing your own work. If YOU are the one who was down, a "(mesh auto-nudge)" message means you were revived — re-check the board and resume where you left off.
